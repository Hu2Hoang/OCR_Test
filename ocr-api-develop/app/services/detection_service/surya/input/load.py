...
from  app.services.detection_service.surya.input.processing import open_pdf, get_page_images, save_image
from  app.services.detection_service.process_image_input import process_image_v2
import os
import filetype
...
def load_image_v2(image_path, type = "pdf"):
    image = Image.open(image_path).convert("RGB")
    type = type.lower()
    # name = get_name_from_path(str(image_path))
    if type == 'pdf':
        return [image]
    else:
        processed_image = process_image_v2(image)
